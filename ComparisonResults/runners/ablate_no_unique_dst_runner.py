...
benign split, three-tier EWMA + CUSUM + JSD + LogZ ensemble, calibrated θ and
fixed-θ=4.0 passes, conjunction baseline-update gate. Per-attack DR is
aggregated across victim subnets (≥30 attack windows), uninvolved-subnet FPR
across subnets with zero attack windows.

Closes the audit's D1 finding by quantifying what the L1 mechanism actually
catches at LITNET scale, alongside the L2-zone numbers run_litnet_zone reports.
...
_orig_filter_active = filter_active
def filter_active(*a, **kw):
    # filter_active returns (active, inert). Iterating the 2-tuple drops nothing
    # and silently scores the 11-feature canonical arm.
    active, inert = _orig_filter_active(*a, **kw)
    out = [f for f in active if f != 'unique_dst_ips']
    print(f"  [ABLATION] dropped unique_dst_ips: {len(active)} -> {len(out)} features")
    if len(out) != 10 or 'unique_dst_ips' in out:
        raise AssertionError(
            f"[GUARD] ablation expected 10 features without unique_dst_ips; "
            f"got {len(out)} {out}")
    print(f"  [GUARD] ablation verified: {len(out)} features, unique_dst_ips absent")
    extra = ['unique_dst_ips'] if 'unique_dst_ips' in active else []
    return out, list(inert) + extra
...
# Results land inside this package (config.RESULTS_DIR), not in a sibling source tree:
# a deposited runner must write where the deposited artifacts are read from.
# The raw-window cache is large and stays outside the package; ANTIDDOS_CACHE_DIR
# overrides it.
CACHE_DIR = os.environ.get("ANTIDDOS_CACHE_DIR",
                           os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "cache"))
# one level up: the package results/ dir, not runners/results/
...
def output_path_for(prefix_bits):
    # Deliberately NOT the canonical artifact path: this runner is an ablation and must
    # never overwrite the shipped subnet results. Writes beside them, under results/rescued/.
    return os.path.join(RESULTS_DIR, "rescued",
                        f"ABLATION_no_unique_dst_l1subnet_{prefix_bits}_results.json")
...
def run_for_prefix(prefix_bits):
    ...
    globals()[f'_CHANGE71_INERT_{prefix_bits}'] = new_inert
    ...
    # with n_uninvolved=0 would mislead a reader skimming the JSON (audit F6).
    ...
    out = {
        'metadata': {
            ...
            'features_inert_auto_excluded': globals().get(f'_CHANGE71_INERT_{prefix_bits}', []),
