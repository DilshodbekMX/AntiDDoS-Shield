...
# --- matched-smoothing overlay (Recommendation 6) -------------------------------
# MS_MATCHED=1 gives EVERY competitor the same EWMA stage the evaluated arm uses,
# so the comparison isolates the subspace residual rather than the smoothing.
# Our arm already carries that stage (subspace_q_ewma), so it is left untouched.
# Matched is the default. MS_MATCHED=0 leaves the registry exactly as deposited:
# that is the scaffolding gate, and it reproduces corrected_benchmark_results.json
# on all 402 numeric leaves except elapsed_sec.
if os.environ.get('MS_MATCHED', '1') == '1':
    _OURS = 'Subspace-Q + EWMA (ours)'
    def _smoothed(fn):
        return lambda a, b: C.ewma(fn(a, b))
    C.DETECTORS = {n: ((f, cit) if n == _OURS else (_smoothed(f), cit + ' [+ EWMA, matched]'))
                   for n, (f, cit) in C.DETECTORS.items()}
...
EXTRACTED = os.path.join(os.environ.get('ANTIDDOS_BASE', '/home/detector/Projects/antiddos'),
                         'datasets', 'extracted')
OUT = os.path.join(RESULTS_DIR, os.environ.get('MS_OUT', 'matched_smoothing_benchmark.json'))
...
def main():
    ...
    mp = os.path.join(RESULTS_DIR, os.environ.get('MS_MATRIX', 'matched_smoothing_matrix.json'))
    ...
    json.dump({'note': 'Per-scenario AUC and benign-episode rates behind every inferential '
